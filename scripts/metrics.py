...
def metrics(pred_path,mask_path):
    global FM
    global WFM
    global SM
    global EM
    ...
    root = os.getcwd()
    mask_root = mask_path
    pred_root = os.path.join(root, pred_path)
    mask_name_list = sorted(os.listdir(mask_root))
    for mask_name in tqdm(mask_name_list, total=len(mask_name_list)):
        mask_path = os.path.join(mask_root, mask_name)
        pred_path = os.path.join(pred_root, mask_name)
        mask_ori = cv2.imread(mask_path, cv2.IMREAD_GRAYSCALE)
        pred_ori = cv2.imread(pred_path, cv2.IMREAD_GRAYSCALE)
        h=pred_ori.shape[1]
        w=pred_ori.shape[0]
        mask_ori=cv2.resize(mask_ori,(h,w))
        _,pred=cv2.threshold(pred_ori,127,255,cv2.THRESH_BINARY)
        _,mask=cv2.threshold(mask_ori,127,255,cv2.THRESH_BINARY)
